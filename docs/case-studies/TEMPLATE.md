...
|------------|-------------|---------------------|
| **ASI-01: Agent Goal Hijack** | Attackers manipulate agent objectives via indirect prompt injection or poisoned inputs | Agent OS policy engine intercepts all actions before execution; unauthorized goal changes blocked in <0.1ms. Policy modes: strict (deny by default), audit (log violations). |
...
| **ASI-08: Cascading Agent Failures** | Initial error or compromise triggers multi-step compound failures across chained agents | Agent SRE circuit breakers; SLO enforcement with error budgets; cascading failure detection; OpenTelemetry distributed tracing. |
